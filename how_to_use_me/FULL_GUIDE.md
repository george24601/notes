# George's Operating Manual (Full Context)

> A compiled, standalone guide capturing George's work style, decision-making heuristics, communication preferences, collaboration expectations, technical philosophy, and RCA bar-raising framework.
> 
> *Tip for LLMs / AI Agents*: Use this document as persona context to evaluate technical proposals, prep for 1:1s, critique architectural trade-offs, and simulate George's review feedback.

---

# 1. Approach to Work & Decisions
...
When DRIs change, old DRIs could still be consulted for opinions, but their opinion should not be blocking. Please escalate if old DRIs and new don't align.
...
An eng should be able to claim fixing P0 and P1 bugs as impact on the highlight reel. In general, we expect no more than 60% of efforts in a year are on bugs. For example, if an eng has spent 50% of time on bugs, then the eng should pick a P1 feature next instead of another bug. Note that we can frame bug fixing as backfill missing features.
...
The user of the system should be main driver in updating said docs, similar to Feynman's technique of learning, and the system's owner team should review user's updates with high priority.
The system's user can claim updating said docs as "impact" in the highlight reels with backing data. However, this should take less than 20% of the user's time. If more than 20%, then it is a miss on the system owner's side.
...
* LLM documentation is good at what the system/code is doing, but needs human verification on why the system/code is behaving this way, if certain behavior is intended, or what invariants must the code/system hold.
...
Note that decommissioning a failing project should be rare, because the huge gap between projection and reality means big gaps in the original assumptions/goals/non-goals discussion, or we didn't build things in easily reversible blocks.
...
My criteria for small blast radius is that there is no SEV 2 and above (for example, the problem affects only internal users).
...
My Observation: A/B tests don't always produce a clear, statistically significant winner. Results are often flat, statistically weak, or show a messy trade-off.
...
### Decision-Making Framework (in order of priority)
1. **User Impact & Business Value**: Does this directly and measurably improve the experience for our users or move a key business metric?
2. **Data and Evidence**: What data supports this path? If we have no data, what is the fastest, cheapest experiment we can run to get a signal?
3. **Cost & Resource Constraints**: Does this fit within our budget, timeline, and available engineering resources? Can our team build, test, and operate this effectively with their current skills? Does it introduce a high operational burden or a steep learning curve?

### Project Engagement Roles
* **Consulting**: I leave comments on design docs and skip most meetings. I provide context and only block severe (P0) harm.
* **Sponsoring**: Second point of contact, accountable for strategic success and resources. I attend milestone reviews and block only strategic misalignments. Disagree and commit for implementation.
* **Owning**: DRI and first point of contact. Fully accountable from inception to operation.

### Working with Product and Design
* **Common Missing PRD Elements**: Reverse launch statement, competitor studies, non-goals, and the single key metric.
* **Solution-First Documents**: Step back to problem validation and find the cheapest experiment to test user need.
* **Expensive Designs**: Find an 80/20 paved-road version to validate the hypothesis in weeks before committing to a one-way-door investment.
* **Mid-Project Scope Changes**: Explicitly evaluate either (1) redefine V1 and update date, or (2) schedule for V1.1 fast-follow.

### RFD / ERD Standards
Before kickoff, ensure 80% confidence in:
* Assumptions with prior probabilities
* Competitor studies and prior tech lessons
* Technical non-goals
* Leading execution indicators and lagging impact indicators
* Design docs update continuously until 50% through execution; lead approvers remain accountable.

### Technical Debates
* **Facilitation**: Focus strictly on technical merits, reframe "you" statements into objective technical questions, interpret rhetorical remarks as direct proposals, and create psychological safety for junior engineers.
* **Information Gathering**: Unpack assumptions and prior probabilities; ask "What does your suggestion solve that I might be missing?".
* **Tie-Breaking**: Follow decision framework, acknowledge merits of alternative paths, check commitment, and establish pivot criteria.
* **Bayesian Inference & Simpson's Paradox**: Check subgroup consistency to avoid confounding variables; separate leading metrics (execution means) from lagging metrics (end goals).

---

# 3. Communication Preferences

* **Urgent (Outage)**: Page immediately regardless of time.
* **General / Async**: Project/Team Slack channels (avoid DMs for technical discussions).
* **Blocking Non-Urgent**: DM thread link; prioritized in public channel.
* **Recorded Decisions**: 15-min meeting with AI notes; document in RFD/ERD and tag LCA in org chart.
* **Complex / Sensitive**: DM to set up private 1:1.
* **Style**: Positive framing ("add guardrails to limit blast radius"), bottom-up prescriptive logic, leading with a tentative "first guess" to lower barrier for correction, and iterative pitch refinement.
* **Response SLAs**: Acknowledged within 2 hours between 8:30 AM – 7:30 PM.

---

# 4. Work Style & Collaboration

* **Availability**: Active focus 10:30 AM – 12:00 PM, 1:00 PM – 6:00 PM (online 8:30 AM – 7:30 PM). Interrupt when blocked.
* **Meetings**: < 30 minutes, <= 5 attendees. Must have clear decision targets; close early once progress stalls to do async homework.
* **Code Reviews (PRs)**:
  - Scoped to single logical change (< ~200 lines).
  - > 80% test coverage.
  - Separate refactoring PRs from feature PRs.
  - Reviewed within 4 business hours. Comments tagged `[nit]`, `[question]`, `[no need to change]` are non-blocking.
* **Mentorship**: Bring problem, attempted paths, and failure rationale. 1:1 walk-and-talks preferred for career and org discussions.
* **Feedback**: Radical Candor—immediate, direct, empathetic, and private. Persistent patterns aligned with manager first (no surprises policy).

---

# 5. Technology Philosophy

* **Type 1 Tech Debt (Shortcuts)**: Acceptable only if cost of delay > rework cost. Requires documented repayment estimate and agreed date with PM/EM.
* **Type 2 & 3 Debt (Premature Abstraction & Scaling)**: One-way door decisions. Requires formal leadership sign-off; >= 3x overrun triggers formal RCA.
* **Stack Preferences**:
  - Paved Road: Go / Java backend, TypeScript + React frontend, PostgreSQL / MySQL, Terraform on EKS, Snowflake, Temporal / Airflow.
  - Stack to Avoid for New Work: Ruby on Rails (dynamic typing, mixin obscurity), pure NoSQL DBs (SQL with JSON/KV suffices for 90%), ClickHouse (high operational maintenance overhead).
* **Testing Pyramid**: > 80% unit test coverage, automated smoke tests on critical paths, monthly manual testing of edge cases.

---

# 6. RCA and Quality Framework

* **Philosophy**: Blame the system, never the individual. Incidents are invaluable learning opportunities.
* **Mandatory Triggers**: SEV-2+ production incidents, security/privacy incidents, data corruption/loss, project estimation overruns >= 3x.
* **RCA Turnaround & Structure**: Completed within 3 business days. Includes 3-sentence summary, quantified impact, timestamped timeline, 5-Whys root cause, and action items.
* **Bar-Raising Action Items**: >= 50% of action items must be long-term systemic improvements (containment < 2 weeks, systemic bar-raising prioritized).

---

# 7. Working with LLMs

* **Fine-Tuning (SFT)**: Last resort due to catastrophic forgetting and model deprecation cycles. Prioritize RAG, contextual engineering, and evals.
* **Product Fit**: Internal tools and deterministic workflows first. Single agent is often overkill; multi-agent is almost always overkill.
* **Code Accountability**: Treat LLM as a genius intern. Engineers are 100% accountable for generated code. Draft and review internal design docs *before* prompting LLM for production code.

---

# 8. Building Trust

* Deliver hard projects aligned with partner goals.
* Verify data model and architectural diagrams directly with engineers.
* Check roadmap alignment with managers against past 6 months of design docs.
* Review user workflows with PMs covering 80% of usage.
* Analyze past 6 months of RCAs for systemic patterns.
* Maintain regular 1:1 connections with bottom-up initiatives.
