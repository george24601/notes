Zero-copy techinique:
normally, disk file -> OS Cache -> App process's cache -> Socket buffer. So Kafka jsut let OS Cache to send to the network card, socket buffer will copy only a descriport but no data (supported by underlying linux)
